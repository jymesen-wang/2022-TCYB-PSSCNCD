...
%% 主函数ClusteringMeasure
% 主函数的输入是两个同维度向量：一个是聚类算法得到的预测标签predY,一个是数据集对应的真实标签GT
% 主函数的输出是三个指标的具体值:ACC,NMI和Purity
% 阅读代码顺序：
% 一.先看15-50行，是计算三个指标的准备工作，记录标签维数以及标签包含类别数量
% 二.阅读51-66行，是Purity的计算原理，与最佳匹配无关，这部分最简单，可以先看
% 三.阅读67-73行，从这几行可以知道，ACC和NMI的计算均是基于最佳匹配算法（匈牙利算法包含在其中）
% 四.重点阅读83-111行最佳匹配算法，弄清楚损失方阵是怎么来的，然后要根据匈牙利算法对损失方阵进行处理
% 五.158行之后的代码都不需要阅读，只需要知道输入是什么输出是什么就好，这一部分需要看一个博客，里面详细讲了匈牙利的算法原理，知道原理就好
% 六.接下来再看一下118-149行的NMI的计算公式就好了，也有相应的博客推荐，最终就可以计算出Purity,ACC和NMI了
% 搞清楚三个指标的计算细节其实对实验结果的分析也是有帮助的


if size(Y,2) ~= 1 %输入如果是行向量就转成列向量
    Y = Y';
end
if size(predY,2) ~= 1 %输入如果是行向量就转成列向量
    predY = predY';
end

n = length(Y); %样本个数

uY = unique(Y);%真实标签的不同标签信息
nclass = length(uY);% 不同的标签数量
Y0 = zeros(n,1); % 初始化n维列向量
if nclass ~= max(Y) % 规整
    for i = 1:nclass
        Y0(Y == uY(i)) = i;
    end
...
pred_classnum = length(predLidx);   % 确定两标签类别数量和具体情况

%% purity 计算原理：从Y中取出对应predY中i的值，取出重复最高的数量Ni，遍历i，得到这个数量的总和，除以n即可
correnum = 0;
for ci = 1:pred_classnum
    incluster = Y(predY == predLidx(ci)); % predY取1时对应Y的值
    %     cnub = unique(incluster);
    %     inclunub = 0;
    %     for cnubi = 1:length(cnub)
    %         inclunub(cnubi) = length(find(incluster == cnub(cnubi)));
    %     end;
    inclunub = hist(incluster, 1:max(incluster)); % 对incluster里面的123进行计数 返回一个行向量
    if isempty(inclunub)
        inclunub=0;
    end
...
%% 利用匈牙利算法进行最佳匹配
%if pred_classnum
res = bestMap(Y, predY);    % 得到最佳匹配对应的向量
%% 计算accuarcy
ACC = length(find(Y == res))/length(Y); % res和Y两向量完全对应相等的数量，除以样本数量
%% 计算NMI
MIhat = MutualInfo(Y,res);  %求出Y和res的互信息



result = [ACC MIhat Purity]; % 求出最终结果





%% 进行最佳匹配：先求出平方损失矩阵G，为了得到最大匹配，那么应该对-G进行最小损失分配（思路反着来）
% 得到的是与标签种类数量相等维度的行向量，该行向量被称作最优匹配结果，也就是主函数中的res，对应代码62行
%% 之后再对predY进行重新赋值，将最佳分配的序号进行分配得到最佳分配predY,之后直接计算相等的元素总数
...
nClass = max(max(L1), max(L2)); % 确定标签类别数
G = zeros(nClass);  % 初始化矩阵G
for i=1:nClass
    for j=1:nClass
        G(i,j) = length(find(L1 == i & L2 == j)); % 计算损失方阵，相当于对于两个矩阵，各标签的配对总数进行记录
    end
end
%===========    assign with hungarian method    ======
[c,~] = hungarian(-G);  % 寻找最大匹配
newL2 = zeros(nClass,1); % 构造新的列向量
for i=1:nClass
    newL2(L2 == i) = c(i); % 按照最佳分配c对predY进行重新分配
end
...
L2 = L2 - min(L2) + 1;      %   min (L2) <- 1;
%===========    make bipartition graph  ============
nClass = max(max(L1), max(L2)); % 样本数
G = zeros(nClass); % 初始化平方损失矩阵
for i=1:nClass
    for j=1:nClass
        G(i,j) = length(find(L1 == i & L2 == j))+eps;% eps是防止后面点除产生奇异值
    end
end
sumG = sum(G(:)); % 矩阵的所有元素之和
%===========    calculate MIhat
P1 = sum(G,2);  P1 = P1/sumG;  % 行和占总和比例 行对应元素的概率分布
P2 = sum(G,1);  P2 = P2/sumG;  % 列和占总和比例 列对应元素的概率分布
H1 = sum(-P1.*log2(P1));       % 行对应元素的个体熵
H2 = sum(-P2.*log2(P2));       % 列对应元素的个体熵
P12 = G/sumG;                  % 行和列对应所有元素的概率分布
PPP = P12./repmat(P2,nClass,1)./repmat(P1,1,nClass); % 点除
PPP(abs(PPP) < 1e-12) = 1;     % 消除很小的值造成的影响
MI = sum(P12(:) .* log2(PPP(:))); % 互信息：联合分布与乘积分布的相对熵
MIhat = MI / max(H1,H2);       % NMI计算公式
%%%%%%%%%%%%%   why complex ?       %%%%%%%%
MIhat = real(MIhat);
...
%% 匈牙利算法：C是最佳匹配，为（1,nclass）维的行向量，下面的代码全部都不用看，只需要知道，输出T对于我们没有用处
% 输出C是我们需要的最佳匹配，也就是主函数中的res,对应代码62行
